...
const version: std.SemanticVersion = .{ .major = 1, .minor = 13, .patch = 1 };
...
pub fn build(b: *std.Build) void {
    ...
    const xkb_legacy_root = b.option([]const u8, "xkb-legacy-root", "The legacy XKB config root") orelse b.pathJoin(&.{ b.install_prefix, "share/X11/xkb" });
    const xkb_config_root = b.option([]const u8, "xkb-config-root", "The XKB config root") orelse xkb_legacy_root;
    ...
    const config_header = b.addConfigHeader(.{}, .{
        ...
        ._GNU_SOURCE = 1,
        .DFLT_XKB_LEGACY_ROOT = xkb_config_root,
